<template>
  <div class="roadmap-create">
    ...
    <div class="right">
      ...
      <form method="post" :action="`/admin/place/edit/${place.id}`" enctype="multipart/form-data">
        ...
        <v-select :items="suggestion"
                  :model-value="place.roadmap_id.title"
                  variant="outlined"
                  name="roadmap_id"
                  label="Roadmap"
                  required/>
...
let suggestion = ref();
let place = ref(window.roadmap);

onMounted(() => {
  let val = window.suggestion;
  let response = [];
  val.forEach((item) => {
    const sug = item.title;
    response.push(sug);
  })
